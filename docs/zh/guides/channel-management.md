...
重试设置中的全局规则与 fork 的渠道/API Key 独立状态码策略分别配置。先检查渠道自定义规则，再检查全局规则；一旦匹配，该规则就拥有本次失败的计数，即使尚未达到阈值也不再交给其他策略处理。未匹配时回退到独立状态码策略，Key 策略优先于渠道策略。渠道模式设为 **关闭** 时跳过全部自动禁用。

升级保留已有临时禁用时长和 429 **Retry-After** 设置。网络故障不会禁用单个 Key，但仍可由独立渠道策略处理。旧配置迁移不会重新开启已明确关闭的策略。

渠道自定义规则和全局规则按 Key、按规则独立累计。穿插其他错误不清零，该 Key 自己请求成功后才清零；同渠道其他 Key 成功不会影响它的计数。
