...
func (m *persistRequestMiddleware) OnOutboundRawRequest(
	ctx context.Context,
	request *httpclient.Request,
) (*httpclient.Request, error) {
	// The response cache is attempt-scoped. Clear it before every outbound request
	// so a later streaming retry cannot reuse a rejected non-stream response.
	m.llmResponse = nil
...
// request metadata without changing the Responses API format used by transformers.
...
func (m *persistRequestMiddleware) OnOutboundLlmResponse(ctx context.Context, llmResp *llm.Response) (*llm.Response, error) {
	...
	state.AppliedServiceTier = llmResp.ServiceTier
...
func (m *persistRequestMiddleware) injectUsageCost(ctx context.Context, resp *llm.Response) {
	...
	if resp.ServiceTier != "" {
		state.AppliedServiceTier = resp.ServiceTier
	}
	...
	state.UsageLogService.InjectUsageCost(
		ctx,
		state.RequestExec.ChannelID,
		state.RequestExec.ModelID,
		resp.Usage,
...
func (s *usageCostStream) Next() bool { return s.stream.Next() }
...
func (s *usageCostStream) Err() error { return s.stream.Err() }

func (s *usageCostStream) Close() error { return s.stream.Close() }

func (m *persistRequestMiddleware) OnInboundRawStream(
	ctx context.Context,
	stream streams.Stream[*httpclient.StreamEvent],
) (streams.Stream[*httpclient.StreamEvent], error) {
	m.inbound.state.UsageLogEligible = true
...
func (m *persistRequestMiddleware) OnInboundRawResponse(ctx context.Context, httpResp *httpclient.Response) (*httpclient.Response, error) {
	...
	_, err := state.UsageLogService.CreateUsageLogFromRequest(
		persistCtx,
		state.Request,
		state.RequestExec,
		llmResp.Usage,
