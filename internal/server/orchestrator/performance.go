...
func (m *performanceRecording) OnOutboundRawRequest(ctx context.Context, request *httpclient.Request) (*httpclient.Request, error) {
	...
	perf.SkipHealthStateTracking = shouldSkipHealthStateTrackingForState(ctx, m.outbound.state)
	...
	if apiKey, ok := contexts.GetChannelAPIKey(ctx); ok {
		perf.APIKey = apiKey
		identity := channel.APIKeyIdentity(apiKey)
		perf.APIKeyName = identity.Name
		perf.APIKeySuffix = identity.Suffix
	}
