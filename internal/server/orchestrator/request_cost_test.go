...
func setupUsageCostMiddleware(t *testing.T, format, modelID string) (*persistRequestMiddleware, context.Context, int) {
	...
	priorityPromptUnit := decimal.NewFromFloat(0.02)
	priorityCompletionUnit := decimal.NewFromFloat(0.04)
			...
			ServiceTierPrices: []objects.ServiceTierPrice{
				{
					...
					Items: []objects.ModelPriceItem{
						{
							ItemCode: objects.PriceItemCodeUsage,
							Pricing:  objects.Pricing{Mode: objects.PricingModeUsagePerUnit, UsagePerUnit: &priorityPromptUnit},
						},
						{
							ItemCode: objects.PriceItemCodeCompletion,
							Pricing:  objects.Pricing{Mode: objects.PricingModeUsagePerUnit, UsagePerUnit: &priorityCompletionUnit},
						},
					},
				},
			},
...
func TestPersistRequestMiddleware_InjectsUsageCostWithEffectiveServiceTier(t *testing.T) {
	t.Parallel()

	middleware, ctx, _ := setupUsageCostMiddleware(t, string(llm.APIFormatOpenAIChatCompletion), "m1")
	state := middleware.inbound.state
	state.RequestedServiceTier = llm.ServiceTierPriority
	state.RequestPricingOverride = llm.ServiceTierPriority
	state.RequestPricingOverridePolicy = biz.RequestPricingOverrideWhenAppliedDefault

	result, err := middleware.OnOutboundLlmResponse(ctx, &llm.Response{
		ID:          "resp-1",
		ServiceTier: llm.ServiceTierDefault,
		Usage: &llm.Usage{
			PromptTokens:     100,
			CompletionTokens: 200,
			TotalTokens:      300,
	...
	require.NoError(t, err)
	require.NotNil(t, result.Usage.Cost)
	require.InDelta(t, 0.00001, *result.Usage.Cost, 1e-12)
}
