	...
	tests := []struct {
		...
		},
		{
			...
			responseBody: `{"id":"resp_incomplete","status":"incomplete","output":[],` +
				...
				`"usage":{"input_tokens":10,"output_tokens":2,"total_tokens":12}}`,
			expectedStatus: requestexecution.StatusFailed,
			expectedError:  "response incomplete: max_output_tokens",
			channelSuccess: true,
		},
		{
			name:      "content filter",
			...
			expectedStatus: requestexecution.StatusFailed,
			expectedError:  "response incomplete: content_filter",
			channelSuccess: true,
		},
		{
		...
		},
		{
			...
			expectedError:   "response canceled: context canceled",
		...
		},
		{
			...
			responseBody:   `{"id":"resp_incomplete_status","status":"incomplete","output":[]}`,
			expectedStatus: requestexecution.StatusFailed,
			expectedError:  "response incomplete",
			channelSuccess: true,
		},
		{
			...
			expectedStatus:   requestexecution.StatusFailed,
			expectedError:    "upstream response failed",
			channelErrorCode: 500,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.channelErrorCode == 0 {
				...
				require.Equal(t, streamTerminalErrorMessage(event, classifyStreamTerminalEvent(event)), perf.ErrorMessage)
			}
...
func TestOutboundPersistentStream_Close_AggregationErrorPreservesTerminal(t *testing.T) {
	tests := []struct {
		...
		{"incomplete", "response.incomplete", `,"incomplete_details":{"reason":"max_output_tokens"}`, requestexecution.StatusFailed, "response incomplete: max_output_tokens"},
		{"canceled", "response.cancelled", "", requestexecution.StatusCanceled, "response canceled: context canceled"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			client := enttest.NewEntClient(t, "sqlite3", "file:ent?mode=memory&_fk=0")
			...
			stream := NewOutboundPersistentStream(streamCtx, &sliceEventStream{
				events: []*httpclient.StreamEvent{event}, err: context.Canceled,
			...
			if tt.expectedStatus == requestexecution.StatusCompleted {
				require.JSONEq(t, `{"existing":true}`, string(saved.ResponseBody))
			} else {
				require.JSONEq(t, string(event.Data), string(saved.ResponseBody))
			}
