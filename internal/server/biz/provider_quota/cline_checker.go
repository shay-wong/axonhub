...
func buildClineQuotaData(now time.Time, scope clineModelScope, threshold clineInferenceCapThreshold, plans []map[string]any, balance *int64, items []clineUsageItem, fetchMeta clineUsageFetchMeta) QuotaData {
	...
	if scope == clineModelScopeMixed && passStatus == "exhausted" {
		statusBasis = "mixed_pool_pass_exhausted_channel_blocked"
	}

	limits := clineLimitStatuses(windows)
	...
	return QuotaData{
		...
		RawData: map[string]any{
			...
			"pool":         "cline_pass",
			"pool_note":    clinePassPoolNote(scope, statusBasis),
			"enforcement":  "channel_token_limit",
...
func buildClineDirectOnlyQuota(balance *int64, plans []map[string]any) QuotaData {
	return QuotaData{
		...
		RawData: map[string]any{
			...
			"pool":         "direct_credit",
			"pool_note":    "Cline (usage-billing) credits balance is informational and does not drive quota enforcement until exact pay-as-you-go exhaustion semantics are verified.",
...
func clinePassPoolNote(scope clineModelScope, statusBasis string) string {
	if scope == clineModelScopeMixed && statusBasis == "mixed_pool_pass_exhausted_channel_blocked" {
		return "ClinePass is a separate provider; because AxonHub enforces quota at channel/token scope, an exhausted ClinePass pool blocks the mixed channel to avoid routing cline-pass/* requests."
	}
	return "ClinePass is a separate provider; this quota applies to cline-pass/* models only."
}

func clineLimitStatuses(windows []clineWindow) []QuotaLimitStatus {
