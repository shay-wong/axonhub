...
func TestCline_CheckQuota_MixedScopeExhaustsWholeChannelFromPassPool(t *testing.T) {
	...
	require.Equal(t, "exhausted", quota.Status)
	require.False(t, quota.Ready)
	require.Equal(t, "mixed_pool_pass_exhausted_channel_blocked", quota.RawData["status_basis"])
	require.Equal(t, "channel_token_limit", quota.RawData["enforcement"])
...
func TestCline_CheckQuota_DirectOnlyUsesBalanceInformationally(t *testing.T) {
	...
	require.Equal(t, "informational_only", quota.RawData["enforcement"])
