...
func (svc *ProviderQuotaService) GetQuotaStatus(ctx context.Context, channelID int) *QuotaChannelStatus {
	...
	if status.ProviderType != "" {
		settings, err := authz.RunWithSystemBypass(ctx, "provider-quota-routing-settings", func(bypassCtx context.Context) (*ProviderQuotaCollectionSettings, error) {
			return svc.SystemService.ProviderQuotaCollectionSettings(bypassCtx)
		})
		if err != nil {
