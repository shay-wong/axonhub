...
func (svc *ChannelService) invalidateProviderQuota(ctx context.Context, channelID int) {
	...
	if err := svc.providerQuotaInvalidator.InvalidateChannelQuota(ctx, channelID); err != nil {
		log.Warn(ctx, "failed to invalidate provider quota after channel quota identity change",
			log.Int("channel_id", channelID),
			log.Cause(err))
	}
}

func (svc *ChannelService) invalidateProviderQuotaAfterCommit(ctx context.Context, channelID int) {
