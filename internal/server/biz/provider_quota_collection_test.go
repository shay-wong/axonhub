...
func TestProviderQuotaService_GetQuotaStatus_UsesSettingsForAPIKeyOnCacheMiss(t *testing.T) {
	service, systemService, ctx, client := setupProviderQuotaCollectionService(t)
	defer client.Close()
	...
	systemService.InvalidateSystemValueCaches(ctx, SystemKeyProviderQuotaCollectionSettings)

	apiKeyCtx := authz.NewAPIKeyContext(ent.NewContext(t.Context(), client), 1, 1)
	require.Nil(t, service.GetQuotaStatus(apiKeyCtx, 1))
}
