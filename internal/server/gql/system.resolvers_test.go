...
func TestQueryResolver_ProviderQuotaCollectionSettingsRequiresReadSettings(t *testing.T) {
	resolver, ctx, client := setupTestSystemMutationResolver(t)
	defer client.Close()

	require.NoError(t, resolver.systemService.UpdateProviderQuotaCollectionSettings(ctx, lo.ToPtr(false), nil))
	_, err := resolver.systemService.ProviderQuotaCollectionSettings(ctx)
	require.NoError(t, err)

	query := &queryResolver{resolver.Resolver}
	apiKeyCtx := authz.NewAPIKeyContext(ent.NewContext(t.Context(), client), 1, 1)
	_, err = query.ProviderQuotaCollectionSettings(apiKeyCtx)
	require.ErrorContains(t, err, "read_settings")
}
