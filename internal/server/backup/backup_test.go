...
func TestBackupService_Backup_SystemConfigs(t *testing.T) {
	...
	for key, value := range map[string]string{
		...
		biz.SystemKeyProviderQuotaCollectionSettings: `{"enabled":false,"providers":{"codex":false}}`,
		biz.SystemKeyWebhookNotifierConfig:           `{"targets":[{"headers":[{"key":"Authorization","value":"webhook-secret"}]}]}`,
		biz.SystemKeyProxyPresets:                    `[{"url":"http://proxy.example.com","password":"proxy-secret"}]`,
	...
	require.NotContains(t, string(data), "webhook-secret")
	...
	data, err = service.Backup(ctx, BackupOptions{IncludeSystemConfigs: true, IncludeAPIKeys: true})
	require.NoError(t, err)
	require.Contains(t, string(data), "webhook-secret")
	require.Contains(t, string(data), "proxy-secret")
}
