...
func TestQuotaRoutingSettings_RestoreNewSettingsDoesNotApplyLegacyMapping(t *testing.T) {
	...
	require.Equal(t, objects.QuotaRoutingModeBackpressure, service.systemService.QuotaRoutingSettingsOrDefault(ctx).DefaultMode)
	marker, err := client.System.Query().Where(system.KeyEQ(biz.SystemKeyQuotaRoutingMigrationDone)).Only(ctx)
	require.NoError(t, err)
	require.Equal(t, "true", marker.Value)
}

func TestQuotaRoutingSettings_RestoreInvalidSettingsRollsBack(t *testing.T) {
	for _, test := range []struct {
		name   string
		config BackupSystemConfig
	}{
		{"legacy invalid JSON", BackupSystemConfig{Key: biz.SystemKeyQuotaEnforcementSettings, Value: `{`}},
		{"legacy invalid mode", BackupSystemConfig{Key: biz.SystemKeyQuotaEnforcementSettings, Value: `{"enabled":true,"mode":"invalid"}`}},
		{"routing invalid mode", BackupSystemConfig{Key: biz.SystemKeyQuotaRoutingSettings, Value: `{"defaultMode":"invalid"}`}},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Error(t, service.Restore(ctx, data, RestoreOptions{IncludeSystemConfigs: true}))
			title, err := client.System.Query().Where(system.KeyEQ(biz.SystemKeyTitle)).Only(ctx)
			require.NoError(t, err)
			require.Equal(t, "Original title", title.Value)
			marked, err := client.System.Query().Where(system.KeyEQ(biz.SystemKeyQuotaRoutingMigrationDone)).Exist(ctx)
			require.NoError(t, err)
