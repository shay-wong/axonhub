...
func TestPriceSchedule_Validate(t *testing.T) {
	...
	t.Run("duplicate override item codes", func(t *testing.T) {
		// Overrides replace the active price list, so duplicates would charge the same usage bucket twice.
		s := &PriceSchedule{
			Timezone: "UTC",
			Overrides: []PriceOverride{
				{
					...
					When:     OverrideWhen{DailyTime: &DailyTimeRange{Start: "00:00", End: "23:59"}},
					Items: []ModelPriceItem{
						{ItemCode: PriceItemCodeUsage, Pricing: Pricing{Mode: PricingModeUsagePerUnit, UsagePerUnit: decPtr("0.01")}},
						{ItemCode: PriceItemCodeUsage, Pricing: Pricing{Mode: PricingModeUsagePerUnit, UsagePerUnit: decPtr("0.02")}},
					},
				},
			},
		}
		...
		err := s.Validate()
		require.EqualError(t, err, `overrides[0]: items[1]: duplicate itemCode "prompt_tokens"`)
	})
}
