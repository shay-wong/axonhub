...
type ChannelSettings struct {
	...
	// Deprecated: retained for API compatibility; the global model setting is used instead.
...
// IsExpiredAt reports whether a temporary API key disable has elapsed.
func (dk DisabledAPIKey) IsExpiredAt(now time.Time) bool {
	expiresAt := dk.DisabledUntil
	if expiresAt == nil {
...
// IsExpired reports whether a temporary API key disable has elapsed now.
func (dk DisabledAPIKey) IsExpired() bool {
	return dk.IsExpiredAt(time.Now())
}
...
func (c *ChannelCredentials) GetEnabledCredentialRefs(disabledKeys []DisabledAPIKey) []string {
	return c.GetEnabledCredentialRefsAt(disabledKeys, time.Now())
}

func (c *ChannelCredentials) GetEnabledCredentialRefsAt(disabledKeys []DisabledAPIKey, now time.Time) []string {
	return filterDisabledAt(c.GetAllCredentialRefs(), disabledKeys, now)
}
...
func filterDisabledAt(candidates []string, disabledKeys []DisabledAPIKey, now time.Time) []string {
	if len(disabledKeys) == 0 {
		return candidates
	}
	...
	for _, candidate := range candidates {
		if _, disabled := disabledSet[candidate]; !disabled {
...
// error-message patterns. At least one condition is required.
//
