...
func (p *ModelPrice) ItemsForServiceTier(serviceTier string) []ModelPriceItem {
	return p.ItemsForServiceTierFromItems(serviceTier, p.Items)
}

// ItemsForServiceTierFromItems merges service-tier overrides over the supplied base items.
func (p *ModelPrice) ItemsForServiceTierFromItems(serviceTier string, baseItems []ModelPriceItem) []ModelPriceItem {
	...
	for idx := range p.ServiceTierPrices {
		if llm.CanonicalServiceTier(p.ServiceTierPrices[idx].ServiceTier) == serviceTier {
			items := append([]ModelPriceItem(nil), baseItems...)
...
func (o *PriceOverride) Validate() error {
	...
	itemCodes := make(map[PriceItemCode]struct{}, len(o.Items))
	for j, item := range o.Items {
		if _, ok := itemCodes[item.ItemCode]; ok {
			return fmt.Errorf("items[%d]: duplicate itemCode %q", j, item.ItemCode)
		}
