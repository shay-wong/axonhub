...
import { buildAPIKeyCredentialsForUpdate, getConfiguredAPIKeys } from '../data/channel-input';
import {
...
export function ChannelsAPIKeyManagementDialog({ open, onOpenChange }: ChannelsAPIKeyManagementDialogProps) {
  ...
  const apiKeyNames = useMemo(() => createAPIKeyNameMap(currentRow?.credentials?.apiKeyConfigs), [currentRow?.credentials?.apiKeyConfigs]);
  ...
  useEffect(() => {
    if (open) {
      setBulkImportText('');
      setLocalKeys(getConfiguredAPIKeys(currentRow?.credentials));
    ...
    // 依赖 currentRow.credentials 会导致导入/删除后的本地更新被覆盖。
  ...
  const handleBulkImport = async () => {
    ...
    try {
      await updateChannel.mutateAsync({
        id: currentRow.id,
        input: {
          credentials: buildAPIKeyCredentialsForUpdate([...validKeys, ...deduped], currentRow.credentials),
        },
      });
  ...
  const deleteKeys = async (keysToDelete: string[]) => {
    ...
    if (active.length > 0) {
      await updateChannel.mutateAsync({
        id: currentRow.id,
        input: { credentials: buildAPIKeyCredentialsForUpdate(remainingKeys, currentRow.credentials) },
      });
    }
