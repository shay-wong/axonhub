...
test('retry settings save global rules independently from fork status policies', async () => {
  const settings = read('features/system/components/retry-settings.tsx');
  const helpers = settings.slice(settings.indexOf('function cloneStatusRules'), settings.indexOf('export function RetrySettings'));
  const submit = settings.slice(settings.indexOf('  const handleSubmit ='), settings.indexOf('  const renderAutoDisablePolicy ='));
  ...
  const formData = {
    autoDisableChannel: {
      enabled: true,
      rules: [{ statusCodes: [403], times: 2, action: 'disable_until_cron', disableUntilCron: '0 0 * * *', disableUntilTimezone: 'Asia/Shanghai' }],
    },
    channelAutoDisable: { enabled: false, statuses: [] },
    apiKeyAutoDisable: { enabled: true, statuses: [{ status: 429, times: 3, action: 'temporary', durationMinutes: 10, useRetryAfter: true }] },
    upstreamErrorPolicy: { mode: 'custom', customMessage: 'Please retry' },
  };
  let saved;
  const handler = new Function('useCallback', 'updateRetryPolicy', 'formData', 'serializeApiKeyAutoDisableRules', 'toApiKeyAutoDisableRuleFormValues', compiled)(
    (callback) => callback,
    { mutateAsync: async (payload) => { saved = payload; } },
    formData,
    serializeApiKeyAutoDisableRules,
    toApiKeyAutoDisableRuleFormValues
  );
  await handler({ preventDefault() {} });
  assert.deepEqual(saved.apiKeyAutoDisable, formData.apiKeyAutoDisable);
  assert.deepEqual(saved.channelAutoDisable, formData.channelAutoDisable);
  assert.deepEqual(saved.upstreamErrorPolicy, formData.upstreamErrorPolicy);
  assert.equal(saved.autoDisableChannel.enabled, true);
  assert.equal(saved.autoDisableChannel.rules[0].disableUntilTimezone, 'Asia/Shanghai');
  assert.equal(saved.autoDisableChannel.rules[0].action, 'disable_until_cron');
});
