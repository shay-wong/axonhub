...
export function DimensionPieCharts({
  ...
  if (isLoading) {
    return (
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        {Array.from({ length: Math.max(3, availableDimensions.length * 3) }).map((_, i) => (
  ...
  const dimensions = [
    { key: 'channel' as const, title: t('analytics.table.channel'), data: channelStats },
    { key: 'model' as const, title: t('analytics.table.model'), data: modelStats },
    { key: 'apiKey' as const, title: t('analytics.table.apiKey'), data: apiKeyStats },
    { key: 'user' as const, title: t('analytics.table.user'), data: userStats },
          ...
          <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
            {dimensions.map((dim) => (
              <PieChartCard
                ...
                title={truncatedDimensions[dim.key]
                  ? `${dim.title} · ${t('analytics.common.topResults', { count: ANALYTICS_DIMENSION_LIMIT })}`
                  : dim.title}
                data={dim.data}
                valueKey={metric.key}
