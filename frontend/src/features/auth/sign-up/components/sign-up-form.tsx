...
import { getAuthenticatedLanding } from '@/features/auth/data/auth-redirect';
...
export function SignUpForm({ className, ...props }: SignUpFormProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const initialProjectID = response.user.projects[0]?.projectID ?? null;
      const landing = getAuthenticatedLanding(response.user, initialProjectID);
      setSelectedProjectId(landing.projectID ?? initialProjectID);
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  className='border-slate-300 !bg-white text-slate-800 placeholder:text-slate-400 focus:border-slate-500 focus:!bg-white focus:ring-2 focus:ring-slate-200'
                  {...field}
                />
              </FormControl>
              <FormMessage />
