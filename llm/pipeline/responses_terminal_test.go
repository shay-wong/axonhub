...
func TestPipeline_ResponsesDisconnectAfterTerminalPreservesOutcome(t *testing.T) {
	tests := []struct {
		...
		{name: "provider reason", eventType: "response.incomplete", status: "incomplete", expectedType: responsestransformer.StreamEventTypeResponseIncomplete, incompleteReason: "provider_limit"},
		{
			...
			errorDetail: &responsestransformer.Error{Type: "server_error", Code: "provider_error", Message: "provider failed", Param: lo.ToPtr("model")},
		},
		// The fork preserves the dedicated cancellation terminal event.
		{name: "cancelled", eventType: "response.cancelled", status: "canceled", expectedType: responsestransformer.StreamEventTypeResponseCancelled},
		{
