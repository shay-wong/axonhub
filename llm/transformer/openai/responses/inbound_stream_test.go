...
func TestInboundTransformer_TransformStream_MapsFinishReasonToCompletedStatus(t *testing.T) {
	tests := []struct {
		name               string
		finishReason       string
		expectedEventType  StreamEventType
		expectedStatus     string
		expectedIncomplete *ResponseIncompleteDetails
	}{
		{name: "length maps to incomplete", finishReason: "length", expectedEventType: StreamEventTypeResponseIncomplete, expectedStatus: "incomplete", expectedIncomplete: &ResponseIncompleteDetails{Reason: "max_output_tokens"}},
		{name: "content_filter maps to incomplete", finishReason: "content_filter", expectedEventType: StreamEventTypeResponseIncomplete, expectedStatus: "incomplete", expectedIncomplete: &ResponseIncompleteDetails{Reason: "content_filter"}},
		{name: "error maps to failed", finishReason: "error", expectedEventType: StreamEventTypeResponseFailed, expectedStatus: "failed"},
		{name: "cancelled maps to canceled", finishReason: "cancelled", expectedEventType: StreamEventTypeResponseCancelled, expectedStatus: "canceled"},
		{name: "canceled stays canceled", finishReason: "canceled", expectedEventType: StreamEventTypeResponseCancelled, expectedStatus: "canceled"},
		{name: "unknown finish reason stays completed", finishReason: "bogus", expectedEventType: StreamEventTypeResponseCompleted, expectedStatus: "completed"},
		{name: "stop stays completed", finishReason: "stop", expectedEventType: StreamEventTypeResponseCompleted, expectedStatus: "completed"},
		{name: "tool_calls stays completed", finishReason: "tool_calls", expectedEventType: StreamEventTypeResponseCompleted, expectedStatus: "completed"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for stream.Next() {
				...
				if ev.Type == tt.expectedEventType && ev.Response != nil {
			...
			require.Equal(t, tt.expectedStatus, *terminal.Status)
			if tt.expectedIncomplete != nil {
				require.NotNil(t, terminal.IncompleteDetails)
				require.Equal(t, tt.expectedIncomplete.Reason, terminal.IncompleteDetails.Reason)
			} else {
...
func TestInboundTransformer_TransformStream_UsageBeforeFinishReasonKeepsMappedStatus(t *testing.T) {
	...
	require.NotNil(t, terminal)
	require.NotNil(t, terminal.Status)
	require.Equal(t, "incomplete", *terminal.Status)
	require.NotNil(t, terminal.IncompleteDetails)
