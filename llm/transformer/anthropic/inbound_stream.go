...
func (s *anthropicInboundStream) flushBufferedReasoningItems() error {
	for _, itemID := range s.pendingReasoningItemOrder {
		...
		delete(s.completedReasoningItems, itemID)
	}
	s.pendingReasoningItemOrder = nil
	return nil
}

func (s *anthropicInboundStream) flushCompletedReasoningItems() error {
	for len(s.pendingReasoningItemOrder) > 0 {
		itemID := s.pendingReasoningItemOrder[0]
		if !s.completedReasoningItems[itemID] {
			return nil
		}
		if err := s.emitBufferedReasoningItem(itemID); err != nil {
			return err
		...
		delete(s.completedReasoningItems, itemID)
		s.pendingReasoningItemOrder = s.pendingReasoningItemOrder[1:]
	}
	return nil
}
...
func (s *anthropicInboundStream) enqueueTerminalEvents() error {
	if s.pendingUsage != nil {
		s.pendingUsage.ServiceTier = s.serviceTier
	}
		...
		if choice.Delta != nil && choice.Delta.ReasoningSignature != nil && *choice.Delta.ReasoningSignature != "" {
			...
			if itemID != "" {
				...
				if responsesReasoningItemDone(chunk.TransformerMetadata) {
					s.completedReasoningItems[itemID] = true
					if err := s.flushCompletedReasoningItems(); err != nil {
